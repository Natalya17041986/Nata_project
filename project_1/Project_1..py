...
fig.write_html("project_1/plotly/histogram_1.html")
...
fig.write_html("project_1/plotly/histogram_2.html")
...
fig.write_html("project_1/plotly/histogram_3.html")

# Чему равна мода распределения? Мода распределения равна 50-55 тысячам рублей
# Каковы предельные значения признака, в каком примерном интервале находится заработная плата большинства соискателей?
# Максимальная зп 8 млн.руб - вероятно выброс. заработная плата большинства соискателей находится в интервале 50-55 т.р.
# Есть ли аномалии для признака заработной платы? Если есть, то какие значения вы бы причислили к таковым? 
# Есть: вся зп от 1 млн.руб
...
fig.write_html("project_1/plotly/histogram_4.html")
...
# Наименьший для неоконченного образования
...
fig.write_html("project_1/plotly/histogram_5.html")
...
fig.write_html("project_1/plotly/bar_1.html")
...
fig.write_html("project_1/plotly/pivot_table_1.html")
...
data_hh_drop = data_hh_dropduplicates.dropna(subset = ['Последняя/нынешняя должность','Последнее/нынешнее место работы'])
...
print(round(data_hh_new['Опыт работы (месяц)'].mean()))
...
outliers = data_hh_new[(data_hh_new['ЗП в рублях'] > 1e6) | (data_hh_new['ЗП в рублях'] < 1e3)]
data_hh_new = data_hh_new.drop(outliers.index)
print(outliers.shape[0])

# В процессе разведывательного анализа мы обнаружили резюме, в которых опыт работы в годах 
# превышал возраст соискателя. Найдите такие резюме и удалите их из данных.

outliers = data_hh_new[data_hh_new['Опыт работы (месяц)']/12 >= data_hh_new['Возраст']]
data = data_hh_new.drop(outliers.index)
print(outliers.shape[0])
